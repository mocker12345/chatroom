...
function joinRoom(socket, room) {
  ...
  socket.emit('roomResult', {room: room});
  socket.setBroadcast.to(room).emit('message', {
...
function handleMessageBroadcasting(socket, nikeNames) {
  socket.on('message', function (message) {
    socket.setBroadcast(message.room).emit('message', {
  ...
  socket.on('nameAttempt', function (name) {
    if (name.indexOf('Guest') === 0) {
      ...
      if (nameUsed.indexOf(name) === -1) {
        ...
        socket.setBroadcast.to(currentRoom.room).emit('message', {
